    ...
    func action(_ action: Action) async {
        ...
        case .onSaveAllButtonPressed:
            for (transportProtocol, result) in state.testResults {
                ...
                let fileName = "Client-\(transportProtocol.rawValue.uppercased())-\(state.scenario)-\(state.distance)-\(Date.now.formatted(date: .numeric, time: .standard).replacingOccurrences(of: ":", with: "_").replacingOccurrences(of: " ", with: "_").replacingOccurrences(of: ",", with: "")).csv"
